...
def load_athletes() -> pd.DataFrame:
    ...
    if not req.issubset(df.columns):
        st.error(f"athletes.csv måste ha kolumnerna: {sorted(req)}")
        st.stop()
...
def load_results(athletes: pd.DataFrame) -> pd.DataFrame:
    ...
    if not req.issubset(df.columns):
        st.error(f"results.csv måste ha kolumnerna: {sorted(req)}")
        st.stop()
...
with tabs[2]:
    ...
    if not ok:
        st.info("Ange admin-lösenordet (ADMIN_PASSWORD i app.py) för att redigera resultaten.")
    else:
...
with tabs[3]:
    ...
    up_picks = st.file_uploader("Ladda upp picks.json för att återställa", type=["json"])
    ...
    up_results = st.file_uploader("Ladda upp results.csv för att återställa", type=["csv"])
